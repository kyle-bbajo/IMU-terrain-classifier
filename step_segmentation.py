...
# ─────────────────────────────────────────────
#
# ★ v8.2 변경: Shank → Foot IMU 기반으로 전환
#   Niswander 알고리즘은 Foot IMU 전용으로 설계됨
#
#   흐름:
#     [Step 1] 전처리   : BPF(1–20Hz) — 드리프트/노이즈 제거
...
#
# ★ 축 매핑: Noraxon MyoMotion Foot 센서
...
# ── 센서 소스 (v8.2: 모두 Foot IMU) ──────────────────────────────
HS_GYRO_SENSOR: str   = "Foot"    # ★ v8.2: Shank → Foot
HS_GYRO_AXIS: str     = "y"       # ML축 회전 (sagittal plane)
HS_ACCEL_SENSOR: str  = "Foot"    # AP Accel 소스
HS_ACCEL_AXIS: str    = "x"       # 충격축 (전후방향)

# ── Niswander et al. (2021) 임계값 ───────────────────────────────
HS_ACCEL_PEAK_SIGMA: float  = 1.8   # AP Accel 후보: mean + 1.8σ (논문 Eq.1)
HS_GYRO_VALID_SIGMA: float  = 0.5   # ML Gyro 검증: 0.5σ (~50–200 °/s)

# 검증 윈도우
HS_FUSION_WINDOW_MS: int    = 125
HS_FUSION_WINDOW_SAM: int   = int(HS_FUSION_WINDOW_MS / 1000 * SAMPLE_RATE)

# ── 하위 호환 (구 코드 참조용, 직접 사용 금지) ──────────────────────
...
def resolve_foot_acc_cols(
    ...
    result: dict[str, str] = {}
    for axis in ("x", "y", "z"):
        exact   = FOOT_ACC_COLS[side][axis]
        pattern = _FOOT_ACC_PATTERNS[side][axis]
        result[axis] = resolve_column(columns, exact, pattern)
    return result
...
def print_config() -> None:
    ...
    print(f"{'='*60}")
    print(f"  Config v8.2 — {'GPU' if USE_GPU else 'CPU'} 모드")
    ...
    print(f"  HS [Niswander2021]: Foot-Accel{HS_ACCEL_AXIS}(>{HS_ACCEL_PEAK_SIGMA}σ)"
