...
# BioMech 피처용 채널 인덱스 (Noraxon 표준 순서 기준)
# 각 센서 그룹: [AccX_LT, AccY_LT, AccZ_LT, GyroX_LT, GyroY_LT, GyroZ_LT,
#               AccX_RT, AccY_RT, AccZ_RT, GyroX_RT, GyroY_RT, GyroZ_RT]
# Z-accel(충격방향): LT=idx2, RT=idx8
FOOT_Z_ACCEL_IDX:  list[int] = [2, 8]   # Foot  Z-accel LT=2, RT=8
SHANK_Z_ACCEL_IDX: list[int] = [2, 8]   # Shank Z-accel LT=2, RT=8
...
# ─────────────────────────────────────────────
# 11. 경로
#   config.py 위치: repo/src/config.py
#   ROOT      → repo/src/
#   REPO_DIR  → repo/
#   PROJECT_DIR → project/
# ─────────────────────────────────────────────
ROOT: Path        = Path(__file__).resolve().parent          # repo/src/
REPO_DIR: Path    = ROOT.parent                              # repo/
PROJECT_DIR: Path = REPO_DIR.parent                         # project/

DATA_DIR: Path  = PROJECT_DIR / "data" / "raw_csv"
BATCH_DIR: Path = PROJECT_DIR / "data" / "processed" / "batches"
H5_PATH: Path   = BATCH_DIR / "dataset.h5"

OUT_DIR: Path      = REPO_DIR / f"out_N{N_SUBJECTS}"
RESULT_KFOLD: Path = OUT_DIR / "kfold"
RESULT_LOSO: Path  = OUT_DIR / "loso"
LOG_DIR: Path      = REPO_DIR / "logs"

for _d in [BATCH_DIR, OUT_DIR, RESULT_KFOLD, RESULT_LOSO, LOG_DIR]:
...
def apply_overrides(
    ...
    if n_subjects is not None:
        ...
        _cfg.OUT_DIR      = _cfg.REPO_DIR / f"out_N{n_subjects}"
