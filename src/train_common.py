...
class H5Data:
    ...
    def X(self) -> np.ndarray:
        """전체 X 데이터를 (N, T, C) float32로 반환한다.
        
        대용량 데이터셋에서는 RAM 사용량에 주의.
        train_kfold 등 하위 호환용.
        """
...
class FlatDatasetPreload(Dataset):
    def __init__(
        ...
        log(f"      FlatDS(Preload/fp16): {self.data.shape}  ({mb:.0f}MB)  ({time.time()-t0:.1f}s)")
...
class BranchDatasetPreload(Dataset):
    def __init__(
        ...
        T     = h5data.T
        chunk = config.DS_CHUNK
        group_parts: dict[str, list[np.ndarray]] = {nm: [] for nm in branch_idx}
    ...
    for ep in range(1, config.EPOCHS + 1):
        try:
            ...
            if "out of memory" in str(e).lower():
                ...
                log(f"  ✗ CUDA OOM at ep{ep}! (BATCH={config.BATCH})")
                if config.USE_GPU:
